...
- 0-30 days, 31-60 days, 61-90 days, 91-120 days, 121+ days
- Medicare claims: 15-day duration (not 30)
- New files pulled every 15-30 days
- Run at least 3 cycles of AR to track collections
...
- Highlight urgent actions like timely filing deadlines
- Always end with a next-step reminder
"""
...
#  Helper — call Groq
# ─────────────────────────────────────────────
def call_groq(user_text: str, pdf_text: str | None = None):
    client = Groq(api_key=st.secrets["GROQ_API_KEY"])

    system = SYSTEM_PROMPT
    if pdf_text:
        system += f"\n\n## UPLOADED PDF CONTENT:\n{pdf_text}\n\nAnswer using this document as your primary reference."

    # Keep last 10 messages to stay within context limit
...
with st.sidebar:
    st.markdown("## ⚕️ AR Denial Assistant")
    st.markdown("*Aayur Solutions · Powered by Groq AI*")
    st.divider()

    st.markdown("### 📎 Upload PDF Document")
    st.caption("Upload any SOP, payer policy, EOB, or remit report.")

    uploaded_file = st.file_uploader(
        label="Choose a PDF", type=["pdf"], label_visibility="collapsed"
    )

    if uploaded_file:
        if st.session_state.pdf_name != uploaded_file.name:
            with st.spinner("Reading PDF..."):
                extracted = extract_pdf_text(uploaded_file.read())
            st.session_state.pdf_text = extracted
            st.session_state.pdf_name = uploaded_file.name
            st.session_state.messages.append({
                "role": "assistant",
                "content": f"📄 **PDF loaded: `{uploaded_file.name}`**\n\nI've read this document ({len(extracted):,} characters extracted). Ask me anything about it!"
            })
            st.rerun()
    ...
    if st.session_state.pdf_name:
        ...
        if st.button("✕ Remove PDF"):
            ...
            st.session_state.messages.append({
                "role": "assistant",
                "content": "PDF removed. Answering from built-in AR/SOP knowledge now."
            })
            st.rerun()
    ...
    st.caption("🔒 HIPAA-compliant. Never enter real patient SSNs.")
...
if st.session_state.pdf_name:
    st.info(f"📄 Reading from: **{st.session_state.pdf_name}**", icon="📎")

if not st.session_state.messages:
    with st.chat_message("assistant", avatar="⚕️"):
        st.markdown("""
👋 **Hello! I'm your AR Denial Resolution Assistant — powered by Groq AI.**
...
if st.session_state.quick_trigger:
    ...
    with st.chat_message("assistant", avatar="⚕️"):
        with st.spinner("Thinking..."):
            reply = call_groq(trigger_text, st.session_state.pdf_text)
        st.markdown(reply)
...
placeholder = (
    ...
    if st.session_state.pdf_name
    else "Describe your denial or ask any AR question..."
)
